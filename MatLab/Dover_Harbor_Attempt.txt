...
u(material_d(:, Ny) == FLOW) = u0;
v(material_d(:, Ny) == FLOW) = -u0;
lid = [2: (Nx-1)]; %lid coordinates
% u(material_d(1,305:Ny) == FLOW) = u0;
% v(material_d(1,305:Ny) == FLOW) = -u0;
% rho(1, 305 : Ny) = rho(2, 305 : Ny);
...
%     ftemp(Nx, :, 6) = ftemp(Nx, :, 8);
    ...
    ftemp(1, 1 : 305, 1) = ftemp(1, 1 : 305, 3);
    ftemp(1, 1 : 305, 5) = ftemp(1, 1 : 305, 7);
    ftemp(1, 1 : 305, 8) = ftemp(1, 1 : 305, 6);
    
    rhow = ftemp(1, 305 : Ny, 9) + ftemp(1, 305 : Ny, 2) + ftemp(1, 305 : Ny, 4) + ...
         2 * (ftemp(1, 305 : Ny, 3) + ftemp(1, 305 : Ny, 6) + ftemp(1, 305 : Ny, 7));
%     ftemp(1, 305 : Ny, 1) = ftemp(1, 305 : Ny, 3) + 2/3 * rhow * u0;
%     ftemp(1, 305 : Ny, 5) = ftemp(1, 305 : Ny, 7) - (1/2) * (ftemp(1, 305 : Ny, 2) - ftemp(1, 305 : Ny, 4))...
%          + 1/6 * rhow * u0;
%     ftemp(1, 305 : Ny, 8) = ftemp(1, 305 : Ny, 6) + (1/2) * (ftemp(1, 305 : Ny, 2) - ftemp(1, 305 : Ny, 4))...
%          + 1/6 * rhow * u0;
     
    ftemp(1,305:Ny,1) = ftemp(1,305:Ny,2) + 2/3*rhow.*v(1,305:Ny); 
    ftemp(1,305:Ny,8) = ftemp(1,305:Ny,6) + 1/2*(ftemp(1,305:Ny,3)-ftemp(1,305:Ny,1))+ ... 
                1/2*rhow.*u(1,305:Ny) - 1/6*rhow.*v(1,305:Ny); 
    ftemp(1,305:Ny,5) = ftemp(1,305:Ny,5) + 1/2*(ftemp(1,305:Ny,1)-ftemp(1,305:Ny,3))- ... 
                1/2*rhow.*u(1,305:Ny) - 1/6*rhow.*v(1,305:Ny); 
    ...
    % Across the lid
    rhon = ftemp(lid, Ny,  9) + ftemp(lid, Ny,  1) + ftemp(lid, Ny,  3) + ...
         2 * (ftemp(lid, Ny,  2) + ftemp(lid, Ny,  5) + ftemp(lid, Ny,  6));
%     ftemp(lid, Ny,  4) = ftemp(lid, Ny,  1) + ftemp(lid, Ny,  3) + ftemp(lid, Ny,  2) + ...
%         2 * (ftemp(lid, Ny,  5) + ftemp(lid, Ny,  6)) - rhon/3 - rhon * u0^2;
%     ftemp(lid, Ny,  7) = rhon/6 + 1/2 * rhon * u0^2 - 1/2 * rhon * u0 - ...
%         ftemp(lid, Ny,  3) - ftemp(lid, Ny,  6);
%     ftemp(lid, Ny,  8) = rhon/6 + 1/2 * rhon * u0^2 + 1/2 * rhon * u0 - ...
%         ftemp(lid, Ny,  1) - ftemp(lid, Ny,  5);
    
    ftemp(lid,Ny,4) = ftemp(lid,Ny,2) - 2/3*rhon.*v(lid,Ny); 
    ftemp(lid,Ny,8) = ftemp(lid,Ny,6) + 1/2*(ftemp(lid,Ny,3)-ftemp(lid,Ny,1))+ ... 
                1/2*rhon.*u(lid,Ny) - 1/6*rhon.*v(lid,Ny); 
    ftemp(lid,Ny,7) = ftemp(lid,Ny,5) + 1/2*(ftemp(lid,Ny,1)-ftemp(lid,Ny,3))- ... 
                1/2*rhon.*u(lid,Ny) - 1/6*rhon.*v(lid,Ny); 
    ...
    if iter == max_iter
        ...
        save(strcat(FILENAME, '_Boundary_Re', int2str(Re)));
        if cont == 'y'
...
% Output ftempal information to command window
fprintf('-------------------------------------\n')
